...
class VisualizationMixin:
    def visualize_network(self, highlight_transition=None, highlight_color='red', 
                          input_frac=None, output_frac=None, old_marking=None):
        ...
        # Добавляем ребра на основе матриц F и H
        for p in range(len(positions_labels)):
            for t in range(len(transitions_labels)):
                if self.F[p][t] > 0:
                    G.add_edge(positions_labels[p], transitions_labels[t], 
                               weight=self.F[p][t], type='input')
                if self.H[t][p] > 0:
                    G.add_edge(transitions_labels[t], positions_labels[p], 
                               weight=self.H[t][p], type='output')
        
        # Определяем позиции узлов с использованием bipartite_layout
        ...
        if enabled_nodes:
            nx.draw_networkx_nodes(G, pos, nodelist=enabled_nodes, 
        ...
        # Рисуем фиксированные метки в позициях
        ...
        # Рисуем движущиеся метки, если в режиме анимации
        if input_frac is not None and highlight_transition is not None:
            t_label = transitions_labels[highlight_transition]
            t_pos = pos[t_label]
            
            # Движущиеся метки на входах
            if input_frac < 1:
        ...
        ax.set_title("Анимированная визуализация сети Петри", fontsize=14, fontweight='bold')
        ax.axis('off')
        
        # Добавляем легенду
        legend_elements = [
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='lightblue', 
                      markersize=15, label='Позиция'),
            plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='lightcoral', 
                      markersize=12, label='Переход'),
            plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='lightgreen', 
                      markersize=12, label='Разрешенный переход'),
            plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='red', 
                      markersize=12, label='Срабатывающий переход')
        ]
    ...
    def draw_tokens(self, ax, position, num_tokens):
        ...
        if num_tokens == 1:
            ...
            ax.text(position[0], position[1], str(num_tokens), 
