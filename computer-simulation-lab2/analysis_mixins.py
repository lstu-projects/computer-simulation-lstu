...
class AnalysisMixin:
    def analyze_network(self):
        ...
        result_text += "P-инварианты помогают анализировать ограниченность, T-инварианты - циклы и живость.\n\n"
        ...
        try:
            # Находим нулевое пространство C^T
            u, s, vt = svd(self.C.T)
            null_mask = s <= 1e-5  # Уменьшаем порог для лучшего обнаружения
            ...
            if null_space.size > 0:
                ...
                for i in range(null_space.shape[1]):
                    ...
                    if np.any(p_inv < 0):
                        p_inv = -p_inv
                    p_inv = np.round(p_inv, decimals=3)  # Округление для читаемости
        ...
        result_text += "Поиск T-инвариантов (решения уравнения C*x = 0, где x - вектор срабатываний переходов):\n"
        result_text += "T-инвариант - это последовательность срабатываний переходов, возвращающая разметку к исходной.\n"
        try:
            u, s, vt = svd(self.C)
            null_mask = s <= 1e-5  # Уменьшаем порог для лучшего обнаружения
            null_space = vt[len(s):, :].T
        ...
        # Проверяем разрешенность переходов в начальной разметке
        enabled_transitions = []
        ...
        result_text += "- Матричный анализ показывает структурные свойства сети, независимые от начальной разметки.\n"
    ...
    def build_reachability_tree_text(self):
        ...
        omega = float('inf')  # Символ бесконечности
        ...
        while queue and step < 50:  # Ограничиваем количество шагов
            step += 1
            ...
            # Для каждого разрешенного перехода создаем новую разметку
            for t in enabled_transitions:
                ...
                for p in range(len(new_marking)):
                    ...
                    # Проверяем условие ω (упрощенная версия)
                    if (new_marking[p] > current_marking[p] and 
                        current_marking[p] != omega and 
                        new_marking[p] > 10):  # Упрощенное условие
                        new_marking[p] = omega
                ...
                if existing_idx == -1:
                    ...
                    # Ссылаемся на существующую разметка
    ...
    def visualize_reachability_graph(self, markings, tree_edges):
        ...
        # Позиционирование (иерархический layout)
        pos = nx.spring_layout(G)
        
        # Рисуем разметки (круги)
        marking_nodes = [n for n, attr in G.nodes(data=True) if attr['node_type'] == 'marking']
        nx.draw_networkx_nodes(G, pos, nodelist=marking_nodes, 
                               node_color='lightblue', 
                               node_shape='o', node_size=500, ax=ax)
        ...
        for t_node in transition_nodes:
            ...
            ax.add_patch(rect)
            ax.text(x, y, t_node, ha='center', va='center', fontsize=8, 
        ...
        legend_elements = [
            ...
            mpatches.Rectangle((0, 0), 0.4, 0.2, linewidth=1, edgecolor='black', 
                              facecolor='lightcoral', label='Переход')
        ]
